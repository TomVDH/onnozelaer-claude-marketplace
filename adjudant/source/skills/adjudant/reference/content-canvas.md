...
> **`adjudant` vault?** Canvases live alongside notes; their parent folder follows the same structural rules. Canvas filenames follow vault naming (kebab-case, no date prefix unless time-relevant). Frontmatter does not apply to `.canvas` files (they are JSON, not Markdown) — but their containing folder may need an `_index.md` per `adjudant:vault-standards (reference/vault-standards.md)`.
...
- For Markdown that goes inside `text` nodes (wikilinks, callouts, embeds): see `adjudant:markdown`.
- For diagrams that would be better as Mermaid than as a hand-laid-out canvas: see `adjudant:mermaid`.
- For vault structural rules around where canvases should live and how to name them: see `adjudant:vault-standards (reference/vault-standards.md)`.
