...
All vault body copy must be **actionable, clear, unambiguous, and short** — voice, tense, structure, callouts, cuts, reference body shapes. Full rules and worked examples in `references/CONTENT_STYLE.md`.

The `PostToolUse` validator hook checks frontmatter. Content style is not mechanically enforced (style is judgment) — `/dream` Pass 2 flags suspected style violations for human review.
